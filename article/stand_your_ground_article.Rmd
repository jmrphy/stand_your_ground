...
dfmaster<-read.csv("cleaned_data/florida_syg_master_data.csv", stringsAsFactors=TRUE)

dfmaster$conviction<-factor(dfmaster$conviction, levels=c("No Conviction", "Conviction"))
dfmaster$victim_unarmed<-factor(dfmaster$victim_unarmed, levels=c("Victim not clearly unarmed", "Victim clearly unarmed"))
dfmaster$victim_crime<-factor(dfmaster$victim_crime, levels=c("Victim was not clearly committing a crime", "Victim was committing a crime"))
dfmaster$could_retreat<-factor(dfmaster$could_retreat, levels=c("Defendant could not clearly have retreated", "Defendant could have retreated"))
dfmaster$accused_weapon<-factor(dfmaster$accused_weapon, levels=c("Defendant did not clearly have a gun", "Defendant clearly had a gun"))
dfmaster$witness<-factor(dfmaster$witness, levels=c("No clear witness(es)", "Clear witness(es)"))
dfmaster$type<-factor(dfmaster$type, levels=c("Non-domestic", "Domestic"))


mastervars<-subset(dfmaster, select=c("victim_initiated", "victim_crime", "victim_unarmed", "defendant_pursued",
                            "could_retreat", "accused_weapon", "deaths", "witness",
                            "physical_evidence", "defendant_property", "type", "victim_race",
                            "victim_gender", "victim_age", "accused_race", "accused_gender",
                            "accused_age", "year"))
...
tableContinuous(mastervars[,sapply(mastervars, is.numeric)],
                stats = c("n", "min", "median", "mean", "max", "s", "na"),
                cap="Summary statistics for numerical variables",
                lab="Summary statistics for numerical variables")

tableNominal(mastervars[,!sapply(mastervars, is.numeric)],
             stats = c("n", "min", "median", "mean", "max", "s", "na"),
                cap="Summary statistics for categorical variables",
                lab="Summary statistics for categorical variables",
                caption.placement = "top",
...
A victim could die from a defendant with a gun, even in cases where the defendant does not intend to kill. A victim is less likely to die from other types of weapons (e.g., a knife) if the defendant is not purposefully intending to kill. Therfore, the effect of victim death on conviction could be conditional on whether the defendant used a gun rather than some other weapon if death by a weapon other than gun indicates a more purposeful and flagrant form of violence. To consider this, I estimate a model similar to Model 1 but with an interaction between *Defendant Gun* and *Deaths*. The model reveals no evidence of an interaction between *Defendant Gun* and *Deaths*. The coefficient for the interaction term is -.66 but statistically indistinguishable from zero (p=.63). Inclusion of the interaction term does not change the interpretation of the race-related coefficients highlighted in the article. 
